...
$parentModule = 'VMDeploy.Orchestrator'
if (-not $ModuleName) { $ModuleName = 'VMDeploy.Orchestrator.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\VMDeploy.Orchestrator'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\VMDeploy.Orchestrator.psd1"
$prereqHash = @{
