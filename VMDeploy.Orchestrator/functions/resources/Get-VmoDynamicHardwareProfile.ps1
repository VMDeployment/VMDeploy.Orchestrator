function Get-VmoDynamicHardwareProfile {
    <#
    .SYNOPSIS
        Read the list of available dynamic hardwareProfiles in the connected SCVMM.
    
    .DESCRIPTION
        Read the list of available dynamic hardwareProfiles in the connected SCVMM.
        Only returns dynamic hardwareProfiles the current user has access to, based on his/her/its role membership.
    ...
    .EXAMPLE
        PS C:\> Get-VmoDynamicHardwareProfile
    ...
    process {
        ...
        foreach ($hardwareProfile in $hardwareProfiles) {
            ...
            Add-Member -InputObject $hardwareProfile -MemberType NoteProperty -Name _Filter -Value $configItem.Filter -Force
			Add-Member -InputObject $hardwareProfile -MemberType NoteProperty -Name _Weight -Value $configItem.Weight -Force
			$hardwareProfile
        }
    }
}