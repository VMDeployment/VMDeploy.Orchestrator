function Get-VmoHardwareProfile {
    ...
    process {
        ...
        foreach ($hardwareProfile in $hardwareProfiles) {
            ...
            if (-not $configItem) { Add-Member -InputObject $hardwareProfile -MemberType NoteProperty -Name _Role -Value 'Admins' -Force }
            else { Add-Member -InputObject $hardwareProfile -MemberType NoteProperty -Name _Role -Value $configItem.Role -Force }
            if ($hardwareProfile._Role -notin $userRoles) { continue }
            $hardwareProfile
        }
    }
}